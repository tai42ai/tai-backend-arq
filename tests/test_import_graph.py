...
``tai_backend_arq`` is on the allowlist. The rule (see the README): the shipped
package imports
the arq broker SDK (arq / redis / croniter), the shared platform
substrate (tai-contract + tai-kit, fastmcp, pydantic, pydantic-settings,
...
The runtime walk imports ``tai_backend_arq`` and every submodule in a fresh
subprocess, then inspects ``sys.modules``. Running it in a subprocess that
imports ONLY ``tai_backend_arq`` means the assertion covers the SHIPPED package's
...
PACKAGE = "tai_backend_arq"
ALLOWED_FIRST_PARTY = frozenset({PACKAGE, "tai_contract", "tai_kit"})

# Every third-party root the shipped ``tai_backend_arq`` graph pulls in -- the
...
def test_shipped_package_imports_only_allowlisted_roots() -> None:
    ...
    assert result.returncode == 0, (
        f"importing the shipped tai_backend_arq graph failed:\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )

    offenders = [line for line in result.stdout.splitlines() if line]
    assert offenders == [], f"non-allowlisted roots in the tai_backend_arq module graph: {offenders}"
...
def test_shipped_sources_name_only_allowlisted_roots() -> None:
    ...
    assert offenders == {}, f"non-allowlisted import roots in the tai_backend_arq sources: {offenders}"
